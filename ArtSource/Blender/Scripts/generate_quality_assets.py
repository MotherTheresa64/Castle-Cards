...
# This is deliberately a source-asset composition pass, not another primitive-mesh pass.
# The existing Castle Cards reference pass still supplies the table, tavern and base terrain;
# this pass replaces/augments the most visible pieces with authored CC0 low-poly assets.
...
def require_sources():
    ...
    if missing:
        raise RuntimeError(
            "Castle Cards quality source assets are missing. "
...
def place_import(path, label, location=(0, 0, 0), rotation_deg=0.0,
    ...
    # Re-origin each imported asset to ground-center before scaling and placement.
...
def build_castle(team):
    ...
    # Wide fortress assembly: central keep + four authored towers + curtain walls. This gives the
    # approved concept's layered silhouette without falling back to cylinders and box merlons.
    place_import(
        CASTLE_CENTRAL[team], f"{team}_CentralKeep",
        location=(0.0, 0.55, 0.0), target_width=3.65
    )

    tower_specs = [
        (TOWER_A[team], f"{team}_FrontTowerL", (-4.15, -0.35, 0.0), 3.05),
        (TOWER_B[team], f"{team}_FrontTowerR", (4.15, -0.35, 0.0), 3.05),
        (TOWER_B[team], f"{team}_RearTowerL", (-3.65, 2.45, 0.0), 2.55),
        (TOWER_A[team], f"{team}_RearTowerR", (3.65, 2.45, 0.0), 2.55),
    ]
    for path, label, location, height in tower_specs:
        place_import(path, label, location=location, target_height=height)
    ...
    for index, (location, width) in enumerate(wall_specs):
        place_import(WALL, f"{team}_Curtain_{index}", location=location, target_width=width)
...
def augment_battlefield():
    ...
    # Retain the existing sculpted board, then add authored high-value terrain pieces. This avoids
    # throwing away the working terrain layout while replacing the most obvious blockout props.
    import_gltf(BASE_TERRAIN)

    place_import(BRIDGE, "HeroBridge", location=(-5.05, -0.55, 0.22), target_width=2.70)

    trees = [
        (TREE_A, (-9.0, -5.6, 0.18), 1.55, -16),
        (TREE_B, (-8.2, -4.7, 0.16), 1.35, 24),
        (TREE_A, (-9.3, 2.6, 0.14), 1.50, 10),
        (TREE_B, (-8.3, 3.7, 0.12), 1.25, -24),
        (TREE_A, (8.8, -6.2, 0.15), 1.52, 18),
        (TREE_B, (9.4, -4.9, 0.14), 1.30, -9),
        (TREE_A, (8.7, 2.1, 0.14), 1.48, -20),
        (TREE_B, (9.4, 3.5, 0.12), 1.28, 31),
        (TREE_B, (6.9, 5.7, 0.10), 1.10, 5),
        (TREE_A, (-7.3, 6.0, 0.10), 1.16, -30),
    ]
    for index, (path, location, height, rotation) in enumerate(trees):
        place_import(
            path, f"AuthoredTree_{index}", location=location,
            rotation_deg=rotation, target_height=height
        )

    rocks = [
        (ROCK_A, (-7.7, -1.7, 0.08), 0.72, 12),
        (ROCK_C, (-7.1, 0.3, 0.06), 0.58, -18),
        (ROCK_C, (7.3, -2.4, 0.06), 0.68, 27),
        (ROCK_A, (7.8, 0.7, 0.05), 0.60, -11),
        (ROCK_C, (5.9, 4.8, 0.05), 0.52, 18),
        (ROCK_A, (-6.1, 4.7, 0.05), 0.55, -8),
    ]
    for index, (path, location, width, rotation) in enumerate(rocks):
        place_import(
            path, f"AuthoredRock_{index}", location=location,
            rotation_deg=rotation, target_width=width
        )
...
def choose_pose_action():
    ...
    # Prefer a relaxed idle so the static NPC never exports in a T-pose.
...
def build_opponent():
    ...
    # A slight forward lean helps the far-side NPC read as an active tabletop opponent instead of
    # an upright display model. The table hides the lower body in the Godot composition.
    root.rotation_euler[0] = math.radians(5.5)
...
def main():
    ...
    augment_battlefield()
    build_opponent()
    print("[CastleCards Quality] Authored CC0 quality pass complete.")
